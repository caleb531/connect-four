...
expect.extend({
  ...
  toBeOneOf: (received, choices) => {
    if (choices.includes(received)) {
      return {
        message: () =>
          `expected ${received} not to be in [${choices.join(', ')}]`,
        pass: true
      };
    } else {
      return {
  ...
  toHavePendingChipAt: async (grid, { column }) => {
    ...
    const { selector, actualColumn } = await pendingChip.evaluate(
      (pendingChipElem) => {
        ...
        return {
          ...
          actualColumn: Math.round(
            parseFloat(translate.slice(translate.indexOf('(') + 1)) / 100
          )
        };
      }
    );
    if (actualColumn === column) {
      return {
        message: () =>
          `expected ${selector} not to be at column ${column} but got ${actualColumn}`,
        pass: true
      };
    } else {
      return {
        message: () =>
          `expected ${selector} to be at column ${column} but got ${actualColumn}`,
        pass: false
      };
    }
  },
  ...
  toHaveChipAt: async (grid, { column, row, chipColor }) => {
    ...
    const chip = nthColumn.locator(
      `.chip.${chipColor}:nth-child(${ROW_COUNT - row})`
    );
    if ((await chip.count()) === 1) {
      ...
      return {
        message: () =>
          `expected ${chipColor} chip to be placed at column ${column}, row ${row}`,
        pass: false
      };
    }
  }

...
test.describe('AI player', async () => {
  test('should avoid losing move (#1)', async () => {
    ...
    utils.placeChips({
      game,
      columns: [
        3, 6, 4, 1, 1, 4, 3, 3, 1, 1, 3, 0, 0, 3, 0, 0, 2, 2, 2, 5, 2, 4, 4, 4,
        5, 0, 2, 2, 4, 6, 6, 6, 6, 6, 3
      ]
    });
  ...
  test('should avoid losing move (#2)', async () => {
    ...
    utils.placeChips({
      game,
      columns: [
        3, 6, 0, 5, 3, 3, 0, 4, 6, 3, 6, 1, 1, 4, 1, 3, 3, 1, 4, 1, 4, 4, 5, 1,
        5, 5, 5, 4, 5, 6, 6, 6, 0, 0, 0
      ]
    });
    return game.players[1].getNextMove({ game }).then((nextMove) => {
  ...
  test('should avoid losing move (#3)', async () => {
    ...
    utils.placeChips({
      game,
      columns: [
        3, 2, 6, 4, 6, 5, 5, 6, 6, 4, 4, 5, 5, 3, 5, 4, 4, 4, 2, 5, 2, 6, 6
      ]
    });
    return game.players[1].getNextMove({ game }).then((nextMove) => {
  ...
  test('should avoid losing move (#4)', async () => {
    ...
    utils.placeChips({
      game,
      columns: [3, 2, 6, 4, 3, 2, 5, 4, 5, 6, 4, 5, 5, 5, 4, 4, 6]
    });
    return game.players[1].getNextMove({ game }).then((nextMove) => {

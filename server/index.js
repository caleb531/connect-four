  ...
  io.on('connection', (socket) => {
      ...
      getRoom(({ room, playerId }, fn) => {
        ...
        if (localPlayer) {
          if (room.players.length === 1) {
            ...
            room.game.pendingNewGame &&
            localPlayer === room.game.requestingPlayer
          ) {
      ...
      getRoom(({ room, column }, fn) => {
        ...
        if (column !== null) {
          ...
          if (room.game.currentPlayer && room.game.currentPlayer.socket) {
            ...
            room.game.currentPlayer.socket.emit('receive-next-move', {
              column
            });
          } else {
    ...
    socket.on(
      ...
      getRoom(({ playerId, room, winner }, fn) => {
        ...
        const submittedWinners = room.players.map(
          (player) => player.lastSubmittedWinner
        );
    ...
    socket.on(
      ...
      getRoom(({ playerId, room, reaction }, fn) => {
        ...
        room.broadcast('send-reaction', {
          reaction,
          reactingPlayer: localPlayer
        });
        fn({});
      })
    );
